...
from app.models import Conversation, EscalationTicket
...
async def _langsmith_escalation_info() -> tuple[str | None, str | None]:
    """Best-effort (run_id, trace_url) for the LangSmith trace covering the
    turn that's escalating right now. run_agent_turn is itself wrapped in
    @traceable, so get_current_run_tree() gives us the run id for free with
    no network call - that's stored on every escalation ticket whenever
    tracing is on. Resolving the actual clickable UI url needs one extra
    LangSmith API round-trip (to look up the project/tenant), so that part
    is best-effort and bounded: if it's slow, unavailable, or tracing is
    off, we just leave the url blank and HR still has the run id to search
    for in the LangSmith project.
    """
    ...
    try:
        ...
        trace_url = await asyncio.wait_for(asyncio.to_thread(_lookup_url), timeout=3.0)
    except Exception:
        logger.warning(f"Could not resolve LangSmith trace url for run {run_id}")
...
@traceable(name="hr_agent_turn", run_type="chain")
async def run_agent_turn(
    ...
    Wrapped in @traceable so that - whenever LANGCHAIN_TRACING_V2 is on -
    every stage of this turn (intent classification, policy retrieval,
    agent tool calls, escalation) lands as one nested trace in LangSmith
    instead of several disconnected runs. classify_intent_label's own LLM
    call and the LangGraph agent's calls below are automatically nested
    under this run via LangSmith's context propagation - no extra plumbing
    ...
    if sensitive_category:
        trace = _build_trace(predicted_intent, sensitive_category, "escalation", None, classify_ms, None, {})
        run_id, trace_url = await _langsmith_escalation_info()
        ticket = await create_escalation_ticket(
            ...
            pipeline_trace=trace, langsmith_run_id=run_id, langsmith_trace_url=trace_url,
        )
    ...
    try:
        ...
        if escalation_ticket_id:
            # The model called escalate_to_hr itself (via bind_tools) rather
            # than hitting one of the safety-net paths below, so the ticket
            # already exists but was created inside the tool call before we
            # knew the final agent_ms/retrieval_debug - back-fill the trace
            # onto that same ticket now instead of creating a second one.
            trace = _build_trace(
                predicted_intent, sensitive_category, "escalation", last_tool_name, classify_ms, agent_ms, retrieval_debug
            )
            run_id, trace_url = await _langsmith_escalation_info()
            ticket = await db.get(EscalationTicket, uuid.UUID(escalation_ticket_id))
            if ticket is not None:
                ticket.pipeline_trace = trace
                ticket.langsmith_run_id = run_id
                ticket.langsmith_trace_url = trace_url
                await db.flush()
            ...
            run_id, trace_url = await _langsmith_escalation_info()
            ticket = await create_escalation_ticket(
                db, employee_id, conversation.id, reason="low_confidence",
                pipeline_trace=trace, langsmith_run_id=run_id, langsmith_trace_url=trace_url,
            )
        ...
        run_id, trace_url = await _langsmith_escalation_info()
        ticket = await create_escalation_ticket(
            db, employee_id, conversation.id, reason="system_error",
            pipeline_trace=trace, langsmith_run_id=run_id, langsmith_trace_url=trace_url,
        )
        result = {
            "route": "escalation",
